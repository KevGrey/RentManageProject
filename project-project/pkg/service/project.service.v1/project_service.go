...
type ProjectService struct {
	...
	projectLogRep          repo.ProjectLogRepo
...
func New() *ProjectService {
	return &ProjectService{
		...
		projectLogRep:          dao.NewProjectLogDao(),
...
func (ps *ProjectService) FindProjectDetail(ctx context.Context, msg *project.ProjectRpcMessage) (*project.ProjectDetailMessage, error) {
	//1.查项目表
	//2，项目和成员的关联表 查项目的拥有者 去member表查名字
	//3。查收藏表 判断收藏状态
	...
	member, err := rpc.LoginServiceClient.FindMemInfoById(c, &login.UserMessage{
		MemId: ownerId,
	})
	if err != nil {
...
func (ps *ProjectService) GetLogBySelfProject(ctx context.Context, msg *project.ProjectRpcMessage) (*project.ProjectLogResponse, error) {
	//根据用户id查询当前的用户的日志表
	projectLogs, total, err := ps.projectLogRep.FindLogByMemberCode(context.Background(), msg.MemberId, msg.Page, msg.PageSize)
	if err != nil {
