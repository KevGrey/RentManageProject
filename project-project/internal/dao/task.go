...
func (t *TaskDao) FindTaskById(ctx context.Context, task int64) (ts *data.Task, err error) {

	session := t.conn.Session(ctx)
	err = session.Where("id=?", task).Find(&ts).Error
	return
}
...
func (t *TaskDao) FindTaskSort(ctx context.Context, code int64, stageCode int64) (v *int, err error) {
	session := t.conn.Session(ctx)
	err = session.Model(&data.Task{}).
		Where("project_code = ? and stage_code=? ", code, stageCode).
