...
import (
	...
	"pomfret.cn/project_api/pkg/model/tasks"
	common "pomfret.cn/project_common"
	"pomfret.cn/project_common/errs"
	"pomfret.cn/project_common/min"
...
func (t *HandlerTask) uploadFiles(c *gin.Context) {
	...
	minioClient, err := min.New("localhost:9009",
		"8BtHXfr0yE9DcbVW",
		"i68zc1qy5qTaY6WCHHG4iZFMwl6uIILD",
		false)
	if err != nil {
	...
	if req.TotalChunks == 1 {
		...
		_, err = minioClient.Upload(context.Background(), bucketName, req.Filename, buf, int64(req.TotalSize), uploadFile.Header.Get("Content-Type"))
		if err != nil {
			c.JSON(http.StatusOK, result.Fail(-999, err.Error()))
			return
		}
	...
	if req.TotalChunks > 1 {
		...
		open.Read(buf)
		formatInt := strconv.FormatInt(int64(req.ChunkNumber), 10)
		_, err = minioClient.Upload(context.Background(), bucketName, req.Filename+"_"+formatInt, buf, int64(req.CurrentChunkSize), uploadFile.Header.Get("Content-Type"))
		if err != nil {
			c.JSON(http.StatusOK, result.Fail(-999, err.Error()))
			return
		}
