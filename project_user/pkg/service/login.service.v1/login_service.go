...
import (
	...
	"pomfret.cn/project-user/internal/database/tran"
	"pomfret.cn/project-user/internal/repo"
	"pomfret.cn/project-user/pkg/model"
	common "pomfret.cn/project_common"
	"pomfret.cn/project_common/encrypts"
	"pomfret.cn/project_common/errs"
	"pomfret.cn/project_common/jwts"
	"pomfret.cn/project_common/tms"
	"strconv"
	"strings"
	"time"
)
...
func (ls *LoginService) GetCaptcha(ctx context.Context, msg *login.CaptchaMessage) (*login.CaptchaResponse, error) {
	...
	//3。生成验证码（随机4位1000-999或者6位100000-999999）
	code := "123456"
	...
	go func() {
		...
		//redis 假设缓存可能存在mysql当中，也可能存在mongo当中，可以能存在memcache当中
		//5。存储验证码 redis当中，过期时间15min
		timeout, cancel := context.WithTimeout(context.Background(), 2*time.Second)
		defer cancel()
		err := ls.cache.Put(timeout, model.RegisterRedisKey+mobile, code, 15*time.Minute)
		if err != nil {
...
func (ls *LoginService) Register(ctx context.Context, msg *login.RegisterMessage) (*login.RegisterResponse, error) {
	...
	//生成一个账户，账户的授权角色是成员 新生成一个角色（如果没有） 同时将此角色的授权node生成
...
func (ls *LoginService) TokenVerify(ctx context.Context, msg *login.LoginMessage) (*login.LoginResponse, error) {
	...
	//if err != nil {
	//	zap.L().Error("TokenVerify FindMemberById error", zap.Error(err))
	//	return nil, errs.GrpcError(model.DBError)
	//}
	//memMsg := &login.MemberMessage{}
	//copier.Copy(memMsg, memberById)
